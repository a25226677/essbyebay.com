  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">Manage withdrawal requests and seller payouts</p>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr><td colSpan={8} className="text-center py-16 text-gray-400"><div className="w-6 h-6 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin mx-auto" /></td></tr>
              ) : items.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-16 text-gray-400"><DollarSign className="size-8 mx-auto mb-2 opacity-30" /><p className="text-sm">No payment requests</p></td></tr>
              ) : items.map((w) => (
                  ...
                  <td className="px-4 py-3 text-right">
                    {w.status === "pending" && (
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => updateStatus(w.id, "approved")} className="p-1.5 text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Approve"><CheckCircle className="size-4" /></button>
                        <button onClick={() => updateStatus(w.id, "rejected")} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg" title="Reject"><XCircle className="size-4" /></button>
                      </div>
                    )}
                  </td>
                </tr>
