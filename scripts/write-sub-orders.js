...
// Shared orders list page generator
function makeOrdersPage(title, orderType) {
  return `"use client";
...
const DELIVERY_STATUSES = [
  ...
  { value: "picked_up", label: "Picked Up" }, { value: "on_the_way", label: "On The Way" },
...
export default function ${title.replace(/ /g, '')}Page() {
  ...
  const fetchOrders = useCallback(async (overrides?: { delivery_status?: string; date?: string; search?: string; page?: number }) => {
    setLoading(true);
    const ds = overrides?.delivery_status ?? deliveryFilter;
    const dt = overrides?.date ?? dateFilter;
    const s  = overrides?.search ?? search;
    const p  = overrides?.page ?? page;
    const params = new URLSearchParams({ page: String(p), limit: "20"${orderType ? `, order_type: "${orderType}"` : ''} });
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={() => { setPage(1); fetchOrders({ page: 1 }); }}
              ...
              <tbody className="divide-y divide-gray-50">
                {orders.map((order) => (
                  <tr key={order.id} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3 text-xs font-semibold text-emerald-600">\${Number(order.profit ?? 0).toFixed(2)}</td>
...
// Seller Orders
const sellerDir = path.join(base, 'app/admin/(dashboard)/sales/seller-orders');
fs.mkdirSync(sellerDir, { recursive: true });
fs.writeFileSync(path.join(sellerDir, 'page.tsx'), makeOrdersPage('Seller Orders', 'seller'), 'utf8');
console.log('Written: seller-orders/page.tsx');

// Inhouse Orders
const inhouseDir = path.join(base, 'app/admin/(dashboard)/sales/inhouse-orders');
fs.mkdirSync(inhouseDir, { recursive: true });
fs.writeFileSync(path.join(inhouseDir, 'page.tsx'), makeOrdersPage('Inhouse Orders', 'inhouse'), 'utf8');
console.log('Written: inhouse-orders/page.tsx');
