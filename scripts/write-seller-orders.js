...
const walletTxApi = `import { getAdminContext } from "@/lib/supabase/admin-api";
...
fs.mkdirSync(path.join(base, 'app/api/admin/customers/wallet-transactions'), { recursive: true });
fs.writeFileSync(path.join(base, 'app/api/admin/customers/wallet-transactions/route.ts'), walletTxApi, 'utf8');
console.log('Written: wallet-transactions/route.ts');


// ── SELLER ORDERS PAGE (full rewrite to match screenshot) ─────────
const sellerOrdersPage = `"use client";

import { useState, useEffect, useCallback, useRef } from "react";
import { useRouter } from "next/navigation";
import { Eye, Download, Trash2, RefreshCw, ChevronLeft, ChevronRight } from "lucide-react";

type OrderItem = { id: string; quantity: number; unit_price: number; line_total: number; profiles: { full_name: string } | null };
type Order = {
  ...
  profit: number; num_products: number; shop_name: string | null; created_at: string;
...
function DeliveryBadge({ status }: { status: string }) {
  const map: Record<string, { bg: string; color: string }> = {
    pending:  { bg: "#fef3c7", color: "#d97706" }, confirmed: { bg: "#dbeafe", color: "#2563eb" },
    picked_up: { bg: "#d1fae5", color: "#059669" }, on_the_way: { bg: "#e0f2fe", color: "#0284c7" },
    delivered: { bg: "#dcfce7", color: "#16a34a" },
  };
  const c = map[status] ?? { bg: "#f3f4f6", color: "#6b7280" };
  const label = status ? status.replace(/_/g, " ").replace(/\\b\\w/g, (x) => x.toUpperCase()) : "—";
  return <span className="inline-flex text-[11px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap" style={{ background: c.bg, color: c.color }}>{label}</span>;
}
...
export default function SellerOrdersPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/users?role=seller&limit=100")
      .then((r) => r.json())
      .then((j) => setSellers(j.items || []));
  }, []);

  const fetchOrders = useCallback(async (overrides?: { seller_id?: string; date?: string; search?: string; page?: number }) => {
    setLoading(true);
    const sl = overrides?.seller_id ?? sellerFilter;
    const dt = overrides?.date ?? dateFilter;
    const s  = overrides?.search ?? search;
    const p  = overrides?.page ?? page;
    const params = new URLSearchParams({ page: String(p), limit: "20", order_type: "seller" });
    if (sl) params.set("seller_id", sl);
  ...
  }, [sellerFilter, dateFilter, search, page]);
  ...
  const downloadInvoice = (order: Order) => {
    ...
    const text = [\`INVOICE - \${order.order_code}\`, \`Customer: \${order.profiles?.full_name ?? "-"}\`,
      \`Date: \${new Date(order.created_at).toLocaleString()}\`, "", lines, "", \`Total: \$\${Number(order.total_amount).toFixed(2)}\`].join("\\n");
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-sm font-bold text-gray-800 shrink-0">Seller Orders</span>
          ...
          <select value={sellerFilter} onChange={(e) => setSellerFilter(e.target.value)}
            className="text-xs border border-gray-200 rounded-lg px-3 py-2 bg-white text-gray-700 focus:outline-none focus:border-orange-400 cursor-pointer min-w-[150px]">
          ...
          <input ref={searchRef} type="text" value={search}
            ...
            onKeyDown={(e) => { if (e.key === "Enter") { setPage(1); fetchOrders({ search, page: 1 }); } }}
          ...
          <button onClick={() => { setPage(1); fetchOrders({ seller_id: sellerFilter, date: dateFilter, search, page: 1 }); }}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-gray-400 gap-2">
            <span className="text-4xl">📦</span><p className="text-sm">No orders found</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm min-w-[1300px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  ...
                  <th className="px-3 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wide">#</th>
                  {["Order Code","Shop","Num. of Products","Customer","Seller","Amount",
                    "Pick Up Status","Delivery Status","Payment Method","Payment Status","Refund","Options"].map((h) => (
              ...
              <tbody className="divide-y divide-gray-50">
                {orders.map((order, idx) => (
                  <tr key={order.id} className="hover:bg-gray-50/60 transition-colors">
                    ...
                    <td className="px-3 py-3 text-xs text-gray-400 font-medium">{offset + idx + 1}</td>
                    <td className="px-4 py-3"><span className="font-mono text-xs font-semibold text-gray-800">{order.order_code}</span></td>
                    <td className="px-4 py-3 text-xs text-gray-600">{order.shop_name || <span className="text-gray-400">—</span>}</td>
                    <td className="px-4 py-3 text-center text-xs font-semibold text-gray-700">{order.num_products ?? 0}</td>
                    <td className="px-4 py-3 text-xs font-medium text-gray-800">{order.profiles?.full_name || <span className="text-gray-400">—</span>}</td>
                    <td className="px-4 py-3 text-xs text-gray-600">{order.seller_profile?.full_name || <span className="text-gray-400">—</span>}</td>
                    <td className="px-4 py-3 text-xs font-bold text-gray-900">\${Number(order.total_amount).toFixed(2)}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => router.push(\`/admin/sales/\${order.id}\`)} title="View"
                          className="p-1.5 rounded-lg" style={{ background: "#fff3e0" }}>
                          <Eye className="size-3.5" style={{ color: "#f97316" }} />
                        </button>
                        <button onClick={() => downloadInvoice(order)} title="Invoice"
                        ...
                        <button onClick={() => handleDelete(order.id)} disabled={deleting === order.id}
                          title="Delete" className="p-1.5 rounded-lg disabled:opacity-50" style={{ background: "#fee2e2" }}>
                          {deleting === order.id
        ...
        {pagination.pages > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => { const p = page - 1; setPage(p); fetchOrders({ page: p }); }} disabled={page <= 1}
              ...
              <button onClick={() => { const p = page + 1; setPage(p); fetchOrders({ page: p }); }} disabled={page >= pagination.pages}
                className="p-1.5 rounded-lg hover:bg-gray-100 disabled:opacity-30 disabled:cursor-not-allowed">
                <ChevronRight className="size-4 text-gray-600" />
              </button>
            </div>
            <span className="text-xs text-gray-400">{pagination.total.toLocaleString()} total orders</span>
          </div>
        )}
      </div>
    </div>
...
fs.writeFileSync(
  path.join(base, 'app/admin/(dashboard)/sales/seller-orders/page.tsx'),
  sellerOrdersPage,
  'utf8'
);
console.log('Written: seller-orders/page.tsx');
